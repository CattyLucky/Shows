...
using Content.Server._Forge.Payroll;
...
public sealed partial class JobReassignmentSystem : EntitySystem
{
        ...
        bool ignoreDemographicRequirements = false)
    {
        ...
        {
            // Some callers want body reassignment to also reissue the carried ID card.
        ...
        _payroll.TrySyncJob(target, data.Job);
    ...
    private void EnsureJobImplants(EntityUid target, JobPrototype job, IEnumerable<EntProtoId>? extraImplants)
    {
        var requiredImplants = job.Special
            ...
            .Select(implantId => implantId.ToString())

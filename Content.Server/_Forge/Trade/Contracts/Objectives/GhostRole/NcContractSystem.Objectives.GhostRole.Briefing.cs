...
public sealed partial class NcContractSystem : EntitySystem
{
    private void AddGhostRoleBriefing(EntityUid mindId, ContractServerData contract)
    {
        ...
        var existingBriefing = briefingComp.Briefing.ToString();
        if (string.IsNullOrWhiteSpace(existingBriefing))
        {
            briefingComp.Briefing = briefing;
            return;
        }

        if (!existingBriefing.Contains(briefing, StringComparison.Ordinal))
            briefingComp.Briefing = existingBriefing + "\n" + briefing;
    }
